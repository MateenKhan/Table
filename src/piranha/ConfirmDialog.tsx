// VENDORED VERBATIM FROM piranha ui/src/pages/tasks/components/ConfirmDialog.tsx — do NOT diverge; at import into piranha, delete src/piranha/ and repoint to the originals.
...
interface ConfirmDialogProps {
  ...
  title: string;
  message: string;
  /** Itemized blast radius — one line per thing destroyed (or explicitly kept). See ConfirmOptions. */
  details?: string[];
  confirmLabel?: string;
  cancelLabel?: string;
  tone?: ConfirmTone;
    ...
    >
      ...
      >
        ...
        {details && details.length > 0 && (
          <ul
            data-feature-id="tasks-confirm-details"
            className="mt-4 max-h-48 overflow-y-auto custom-scrollbar rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 space-y-1"
          >
            {details.map((d, i) => (
